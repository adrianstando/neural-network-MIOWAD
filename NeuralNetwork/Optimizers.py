import numpy as np
from .Regularization import RegularizationL1
...
class SGD_regularised(Optimizer):
    def __init__(self, learning_rate=1e-3, regularization = RegularizationL1()):
        self.learning_rate = learning_rate
    ...
    def step(self, X, Y, weights, biases, backward_func):
        def changes(weight_grads, biases_grads, lr):
            ...
            new_biases = []
            for i in range(len(weight_grads)):
                new_weights.append((-1) * lr * (weight_grads[i] + self.regularization.derivative(weights[i])))
                new_biases.append((-1) * lr * biases_grads[i])
...
class GradientDescent_regularised(Optimizer):
    def __init__(self, learning_rate=1e-3, regularization = RegularizationL1()):
        self.learning_rate = learning_rate
        self.regularization = regularization
...
class GradientDescentBatch_regularised(Optimizer):
    def __init__(self, learning_rate=1e-3, batch_size=32, regularization = RegularizationL1()):
        self.learning_rate = learning_rate
        self.batch_size = batch_size
        self.regularization = regularization
